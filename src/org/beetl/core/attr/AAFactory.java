...
public class AAFactory {
	...
	static public AA buildFiledAccessor(Class c, String attrExp) {
		...
		if (pojoResult != null) {
			...
			if (aa != null) {
				...
				aa = ASMUtil.instance().createAAClass(pojoResult.c, name,
						pojoResult.realMethodName, pojoResult.returnType);
			...
			if (aa != null) {
				...
				if (generaGetResult != null) {
					aa = ASMUtil.instance().createAAClass(generaGetResult.c,
	...
	public static FindResult findCommonInterfaceOrClass(Class c, String name) {
		...
		mbuffer.append(name.substring(0, 1).toUpperCase()).append(
				name.substring(1));
