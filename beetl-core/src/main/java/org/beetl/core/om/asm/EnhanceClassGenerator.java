...
class EnhanceClassGenerator implements Opcodes {
	...
	static byte[] generate(Class<?> beanClass) throws Exception {
		return generate(beanClass.getName());
	}
	 ...
	 * @throws Exception
	 */
	protected static byte[] generate(String beanClassName) throws Exception {
		return generate(beanClassName, BeanEnhanceConstants.SUPER_CLASS_NAME, null);
	}
	 ...
	 * @throws Exception
	 */
	static byte[] generate(String beanClassName, String superName, String[] interfaces) throws Exception {
			...
			for (int i = 0; i < lookupSwitchLabels.length; i++) {
				...
				if (fieldNodes.size() == 1) {
					...
					mv.visitMethodInsn(INVOKEVIRTUAL, internalClassName,
							...
							false);
					addInvokeValueOfToPrimitive(mv, curFieldNode);
	...
	private static void addInvokeValueOfToPrimitive(MethodVisitor mv, FieldNode curFieldNode) {
		switch (curFieldNode.desc) {
